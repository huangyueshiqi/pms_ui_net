<!--
 * @Descripttion: (动作管理/ui_animation)
 * @version: (1.0)
 * @Author: (admin)
 * @Date: (2022-08-12)
 * @LastEditors: (admin)
...
<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" v-hasPermi="['resourcemng:uianimation:add']" plain icon="el-icon-plus" size="mini" @click="handleAdd">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" :disabled="single" v-hasPermi="['resourcemng:uianimation:edit']" plain icon="el-icon-edit" size="mini" @click="handleUpdate">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" :disabled="multiple" v-hasPermi="['resourcemng:uianimation:delete']" plain icon="el-icon-delete" size="mini" @click="handleDelete">删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" v-hasPermi="['resourcemng:uianimation:export']">导出</el-button>
      </el-col>
    ...
    <el-table :data="dataList" v-loading="loading" ref="table" border highlight-current-row @sort-change="sortChange" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="uiIndex" label="序号" align="center" />
      <el-table-column prop="animID" label="动作编号" align="center" />
      ...
      <el-table-column label="操作" align="center" width="140">
        <template slot-scope="scope">
          <el-button size="mini" v-hasPermi="['resourcemng:uianimation:edit']" type="success" icon="el-icon-edit" title="编辑"
            @click="handleUpdate(scope.row)"></el-button>
          <el-button size="mini" v-hasPermi="['resourcemng:uianimation:delete']" type="danger" icon="el-icon-delete" title="删除"
    ...
    <el-dialog :title="title" :lock-scroll="false" :visible.sync="open" >
      <el-form ref="form" :model="form" :rules="rules" :label-width="formLabelWidth">
        <el-row :gutter="20">
          ...
          <el-col :lg="12">
            <el-form-item label="序号" prop="uiIndex">
              <el-input v-model="form.uiIndex" placeholder="请输入序号" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="动作编号" prop="animID">
              <el-input v-model="form.animID" placeholder="请输入动作编号" />
            </el-form-item>
          </el-col>
...
import {
  ...
  getUiAnimation,
  exportUiAnimation,
} from '@/api/resourcemng/uiAnimation.js';
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { index: 2, key: 'uiIndex', label: `序号`, checked:  true  },
        { index: 3, key: 'animID', label: `动作编号`, checked:  true  },
        { index: 4, key: 'iconRes', label: `默认图标`, checked:  true  },
        { index: 5, key: 'selectedIconRes', label: `选中图标`, checked:  true  },
        { index: 6, key: 'iconResID', label: `资源ID`, checked:  true  },
        { index: 7, key: 'selectedIconResID', label: `选中资源ID`, checked:  true  },
      ],
      // 数据列表
      dataList: [],
      // 总记录数
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listUiAnimation(this.queryParams).then(res => {
         if (res.code == 200) {
    ...
    handleExport() {
      ...
      this.$confirm("是否确认导出所有动作管理数据项?", "警告", {
        ...
        .then(function () {
          return exportUiAnimation(queryParams);
        })
        .then((response) => {
